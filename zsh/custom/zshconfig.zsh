...
function prompt_elements {
    ...
    if which rvm_prompt > /dev/null 2>&1; then
        elements+=(rvm)
    fi
...
export PATH="$HOME/bin:$HOME/.rvm/bin:/bin:/usr/bin:/sbin:/usr/sbin"
export VIM_BUNDLE_DIR="$HOME/.vim/bundle"
POWERLEVEL9K_LEFT_PROMPT_ELEMENTS=($(prompt_elements))
POWERLEVEL9K_PROMPT_ON_NEWLINE='true'
RPROMPT=''

# Configure VIM_RUNTIME_DIR for use in .vimrc.
if [[ -d "$DOTFILE_DIR/vim/vimdir" ]]; then
    # We want to use $DOTFILE_DIR if it's available - that will reflect what
    # is in git.
    VIM_RUNTIME_DIR="$DOTFILE_DIR/vim/vimdir/local"
else
    # If $DOTFILE_DIR doesn't exist, we're probably running as a different
    # user. They will probably have their .vim folder symlinked to my personal
    # folder, so use that.
    VIM_RUNTIME_DIR="$HOME/.vim/local"
fi
export VIM_RUNTIME_DIR
...
cygwin="$(/bin/uname -a 2>/dev/null | /bin/grep CYGWIN 2>&1)"
if [[ -n "$cygwin" ]]; then
...
# RVM Configuration
rvm_config=/etc/profile.d/rvm.sh
[[ -s "$rvm_config" ]] && source "$rvm_config"
